defmodule Pooly.PoolServer do
  ...
  defmodule State do
    defstruct pool_sup: nil, worker_sup: nil, monitors: nil, size: nil, workers: nil, mfa: nil, name: nil
  end
  ...
  def init([pool_sup, pool_conifg]) when is_pid(pool_sup) do
    Process.flag(:trap_exit, true)
    monitors = :ets.new(:monitors, [:private])
    init(pool_conifg, %State{pool_sup: pool_sup, monitors: monitors})
  end
  ...
  def handle_cast({:checkin, worker}, %{workers: workers, monitors: monitors = state}) do
    case :ets.lookup(monitors, worker) do
      [{pid, ref}] ->
        ...
        {:noreply, %{state | workesr: [pid|workers]}}
      [] ->
        {:noreply, state}
     end
    end
